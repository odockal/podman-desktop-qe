...
2. Install a new extension from OCI Image -> `ghcr.io/projectatomic/ai-studio:latest` (possible to also test `:nightly` for latest builds - every commit from `https://github.com/projectatomic/ai-studio`)
...
4. Install demo application - will download a rather large LLM model from HuggingFace
